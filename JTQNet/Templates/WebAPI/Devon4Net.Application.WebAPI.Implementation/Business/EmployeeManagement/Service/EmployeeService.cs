...
namespace Devon4Net.Application.WebAPI.Implementation.Business.EmployeeManagement.Service
{
    ...
    public class EmployeeService : Service<EmployeeContext>, IEmployeeService
    {
        ...
        {
            Devon4NetLogger.Debug(
                $"SetEmployee method from service Employeeervice with value : {name}, {surName}, {mail}");
        ...
        public async Task<Employee> ModifyEmployeeById(long id, string name, string surName, string mail)
        {
            ...
            if (employee == null)
            {
                throw new EmployeeNotFoundException(
                    $"The employee with id {id} does not exists and is not possible to delete.");
            }
